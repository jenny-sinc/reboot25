...
const FormFeature = () => {
    ...
    const handleProductToggle = (product) => {
        setSelectedProducts((prevSelected) =>
            prevSelected.includes(product)
                ? prevSelected.filter((item) => item !== product) // Deselect if already selected
                : [...prevSelected, product] // Add to selected if not already selected
        );
    };
    ...
    const handleSubmit = () => {
        const selectedAccountNames = selectedAccounts.map((id) =>
            accounts.find((account) => account.id === id)?.name
        ).join(', ');

        const selectedProductNames = selectedProducts.join(', ');

        const payload = `I have the following financial products in my portfolio: ${selectedAccountNames || 'None'}\nI'm interested in learning more about the following products that may benefit me: ${selectedProductNames || 'None'}\nWith a particular focus on the following points: ${freeTextInput || 'None'}`;

        console.log('Payload:', payload);

        setPrompt(payload);

        redirectToStep('/recommended-products');
    };
                        ...
                        <div>
                            ...
                            <div className="checkbox-group">
                                <label className="checkbox-item">
                                    <input
                                        type="checkbox"
                                        onChange={() => handleProductToggle('Savings')}
                                    />
                                ...
                                <label className="checkbox-item">
                                    <input
                                        type="checkbox"
                                        onChange={() => handleProductToggle('Mortgages')}
                                    />
                                    <span className="custom-checkbox"></span>
                                    Mortgages
                                </label>
                                <label className="checkbox-item">
                                    <input
                                        type="checkbox"
                                        onChange={() => handleProductToggle('Current account')}
                                    />
                                    <span className="custom-checkbox"></span>
                                    Current account
                                </label>
